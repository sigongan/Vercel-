...
- **Payment note**: Avocato Pro is sold entirely through Apple's In-App Purchase (StoreKit) — see `docs/ios-storekit-setup.md`. There's no Stripe or any other payment path anywhere in the app or on the website; on the web, the same purchase buttons show "Get Avocato Pro in the iPhone app" instead. The IAP subscription product must be created in App Store Connect and set to "Ready to Submit" before the binary can be submitted.
- **Account deletion**: required by Guideline 5.1.1(v) since the app supports account creation — already built (Profile → Delete Account, `app/api/account/delete/route.ts`). Test this in TestFlight before submitting.
- **Review notes**: account creation is Sign in with Apple only (no email/password, no other third-party login) — reviewers can sign in with their own Apple ID, no test account needed.
