...
**Avocato는 결제를 애플 인앱결제(StoreKit)로만 처리해요 — Stripe는 완전히
제거했어요.** 웹에는 결제 수단이 없고, Pro 구독은 iOS 앱에서만 구매할 수
...
이 앱을 웹 브라우저(사파리 등)에서 열었을 때는 구독 버튼 대신 "Get
Avocato Pro in the iPhone app" 안내만 떠요. 실제 결제는 iOS 앱 안
StoreKit 한 곳에서만 일어나요. 이미 iOS에서 Pro로 가입한 계정은
웹에서 로그인해도 저장한 레시피 등 Pro 기능이 그대로 보여요(같은
계정 기준) — 웹에서 새로 구매하는 경로만 없는 거예요.
