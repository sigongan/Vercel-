...
export function GroceryListSheet({
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex flex-col justify-end bg-black/40" onClick={onClose}>
      <div
        className="flex max-h-[85vh] flex-col rounded-t-3xl bg-[#FAFAF7] dark:bg-stone-800 shadow-[0_-8px_30px_rgba(0,0,0,0.25)]"
      ...
      >
                ...
                <ul className="flex flex-col gap-1">
                  {group.map((item) => (
                    <li key={item.id} className="flex items-center gap-3">
                      <label className="flex flex-1 cursor-pointer items-center gap-3 rounded-xl px-2 py-2 transition-colors hover:bg-white/60 dark:hover:bg-stone-700/60">
                        <input
                          type="checkbox"
        ...
        {items.length > 0 && (
          <div className="flex gap-2 border-t border-[#E2E6D9] dark:border-stone-700 px-6 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
            <button
              onClick={() => {
                hapticTap();
            ...
            {hasChecked && (
              <button
                ...
                className="flex-1 rounded-full border border-[#E2E6D9] dark:border-stone-600 bg-white dark:bg-stone-900 py-2.5 text-sm font-medium text-[#5E7A33] dark:text-stone-300 transition-colors hover:border-[#C0DC8C]"
              >
